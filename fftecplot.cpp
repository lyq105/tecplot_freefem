...
double savetec3d(string* const& file, Mesh3* const& pTh,KN<double>* const& uh,string* const& varname)
{
	string shape;
  ofstream pf((*file).c_str());
  size_t n, m;
  ...
  */
  ...
  pf << "ZONE N=" << Th.nv  << ", E=" << Th.nt << ", F=FEPOINT, ET=" << shape << endl;
  for (int i=0;i<Th.nv;i++)
    pf << std::setprecision(5) << setw(18) << (R3) Th(i)  << " " << (*uh)[i] <<" \n" ;
...
double saveTecplot3d(string* const& file, Mesh3* const& pTh,KN<double>* const& uh)
{
	string shape;
  ofstream pf((*file).c_str());
  size_t n, m;
                 ...
                 )
{
  // 提取网格数据
	...
	double* z = new double[th.nv];
	double* u = new double[th.nv];
 	INTEGER4* cont = new INTEGER4[4*th.nt];
	for (int j=0;j<th.nt;j++){
		for (int i=0;i<4;i++){
			cont[j*4+i]=th(j,i)+1;
			//cout<< cont[j*4+i] << " ";
...
Init::Init(){
	...
	Global.Add("savetec","(",new OneOperator3_<double, string*, Mesh3*,KN<double>*>(saveTecplot3d));
	Global.Add("savetec","(",new OneOperator4_<double, string*, Mesh3*,KN<double>*,string*>(savetec3d));
	Global.Add("savetecb","(",new OneOperator4_<double, string*, Mesh3*,KN<double>*,string*>(savetec_b3d));
	Global.Add("savetecb","(",new OneOperator5_<double, string*, Mesh3*,KN<double>*,KN<double>*,string*>(savetec_b23d));
	Global.Add("savetecb","(",new OneOperator6_<double, string*, Mesh3*,KN<double>*,KN<double>*,KN<double>*,string*>(savetec_b33d));
	Global.Add("savetecb","(",new OneOperator7_<double, string*, Mesh3*,KN<double>*,KN<double>*,KN<double>*,KN<double>*,string*>(savetec_b43d));
}
